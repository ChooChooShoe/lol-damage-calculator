<template>
    <div class="statsdiv">
        <div :class="key" v-for="(val, key) in statsTotal.stats" :key="key">+{{ val }}{{ stats_to_name[key] }}</div>
...
const statsTotal = computed(() => {
    ...
    for (const statMod of props.statmods) {
        for (const [stat, val] of Object.entries(statMod.stats) as [keyof StatBlock, number | string | number[]][]) {

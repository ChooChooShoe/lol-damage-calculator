<template>
  <span :class="data.color" :title="val.stat || 'base'">
    ...
    <RecursiveRatioDisplay
      ...
      :val="v"
      :key="v.stat"
      v-for="(v, k) in val.sub_calcs || val.sub_ratios"
    >
    </RecursiveRatioDisplay>
    {{ dispPost }}
...
const data = computed(
  () => stat_to_display[props.val.stat || 'base'] || default_spell_ratios
);
...
const values = computed(() => {
  switch (props.display) {
    case 'value': {
      if (
        props.val.apply === 'based_on_level' &&
        Array.isArray(props.val.values)
      )
    ...
    case 'dmg_premitigation':
      return (
        Math.round(props.val.damagePreValue.value) ||
        Math.round(props.val.damagePreValue) ||
        0
      );
    case 'dmg_postmitigation':
      return (
        Math.round(props.val.damagePostValue.value) ||
        Math.round(props.val.damagePostValue) ||
        0
      );
    default:
      return 0;

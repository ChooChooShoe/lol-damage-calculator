...
export function makeVarsForEffects(text, extra_data, config) {
    ...
    while (looping) {
        ...
        text = text.replace(INNER_BRACE, (_match, capture, _offset, _fulltext) => {
            ...
            for (const par of split.slice(1)) {
                ...
                if (has_eq)
                    named_parameters[has_eq[1]] = has_eq[2];
                else
                    positional_argument.push(par);
            }
...
export function numberExpand(slices, maxrank = 5, fractionDigits = 3) {
...
export const match_lookup = {
    st: ({ capture, slices, options, vars, extra_data }) => {
        return capture;
    },
    // ap (or Ability progression): {{ap|<Value1>|<Value2>|<...>|<Value6>}}
    ap: ({ capture, slices, options, vars, extra_data }) => {
        const round = parseInt(options.round) || 3;
        const maxrank = (extra_data === null || extra_data === void 0 ? void 0 : extra_data.maxrank) || 5;
    ...
    as: ({ capture, slices, options, vars, extra_data }) => {
        return capture;
    },
};
console.log(makeVarsForEffects(text, null));
console.log("makeVarsForEffects(text, null)");
//# sourceMappingURL=matchReplaceVars.js.map
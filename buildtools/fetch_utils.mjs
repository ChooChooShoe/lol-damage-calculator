...
if (DEBUG) {
    ...
    fs.mkdirSync(`./.debug/`, { recursive: true }, (err) => { if (err && err.code !== 'EEXIST') console.info(err) })
}
...
export async function fetch_mod_data() {
    const raw = await fetch_wiki(`https://leagueoflegends.fandom.com/wiki/Module:ChampionData/data?action=edit`);
    ...
    if (DEBUG)
        saveFile("./.debug/Module_ChampionData.json", x);
    return x;
}

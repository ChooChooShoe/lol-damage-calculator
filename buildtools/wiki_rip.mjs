...
async function makeChampionList(ModuleChampionData) {
  ...
  for (const [champ, data] of Object.entries(ModuleChampionData)) {
    ...
    promises.push(
      fetch_live_wiki_skills(champ)
        .then(async (skills_model) => {
  ...
  if (DEBUG)
    saveFile('./src/api/ChampionListComplete_DEBUG.json', ChampionList);
  else {
...
export async function fetch_live_wiki_skills(champ_name) {
  ...
  const all_skills_div = [...document.querySelectorAll(`.skill`)].flatMap(
    (div) => {
      const skill_name = div.className
        .replaceAll(/skill/g, '')
        .replaceAll(/[_\s]/g, '');
      const infoTabs = [
        ...div.nextElementSibling.nextElementSibling.querySelector('.tabber')
          .children,
      ].slice(2);
      ...
      return Object.entries(subs).map(([idx, subspell]) => {
        ...
        return {
          ...
          skill_idx,
          div: subspell,
          header_aside: header_aside.item(idx),
  ...
  for (const {
    ...
    const header = div.querySelector('.champion-ability__header');
    skills[name].display_name = header.querySelector('h3').textContent;

    for (const child of header_aside.children) {
      const z = child.getAttribute('data-source');
      const key = z.replaceAll(/ /g, '_');
    ...
    skills[name].img = [...div.querySelectorAll('img')]
      .map((x) => x.outerHTML)
      .map((s) => {
    ...
    skills[name].leveling = [...div.querySelectorAll('.skill_leveling')]
      ...
      .flatMap((leveling, levelingidx) => {
        ...
        for (const idx in dt_list) {
          const fulltext = dd_list[idx];
          // let ratios = fulltext.split(/[\(\)+]+/).filter(x => x.trim() !== '');
          const root = parenthesis.parse(fulltext);
          const ratio_root = { name: dt_list[idx], raw: fulltext };
...
function makeRatioObj(root) {
  ...
  const s = fulltext.replace(/[()+]/g, '').split('%');
  const ispercent = s.length > 1;
  const values = arrOrNum(levelingToArray(s[0]));
  const stat_raw = s.slice(1, s.length).join('%');
  ...
  return {
    ...
    apply,
    stat_raw: fulltext,
    sub_ratios: sub_ratios.length === 0 ? undefined : sub_ratios,
  };
}

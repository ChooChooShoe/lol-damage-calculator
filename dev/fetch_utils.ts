...
export async function fetchAndSaveRealms(): Promise<{ v: string, l: string, cdn: string }> {
    const realms: any = await (await fetch("https://ddragon.leagueoflegends.com/realms/na.json")).json();
...
export function saveFile(path: fs.PathLike | fs.promises.FileHandle, data: any): Promise<void> {
...
export async function fetch_wiki(url: RequestInfo) {
    ...
    if (text)
        return { text: text.textContent || "", response }
...
export async function make_wiki_skill_model(champ_name: string, skill_name: string) {
...
function autoCast(s: string | null | undefined): string | number | boolean | null {

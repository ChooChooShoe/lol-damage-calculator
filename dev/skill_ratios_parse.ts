...
import { SubRatio } from "../src/api/DataTypes.js";
...
function table_check<T>(table: { [key: string]: T }, search: string, fallback: T): T {
...
function units_to_user_and_stat(unit: string | undefined | null): { user: 'none' | 'player' | 'target'; stat: string; } {
    ...
    if (player_stat === 'hp')
        ...
        bbt = table_check(keyword_to_type, tunit, "total");
    let stat = `${bbt}_${player_stat}`;

    let user: 'player' | 'target' = tunit.includes('target') ? 'target' : 'player';
    return { user, stat };
}
...
export function makeRatioObj(root: ArrayTree): SubRatio {
    let pre_arr: any = [], sub_ratios: SubRatio[] = [], post_arr: any = [];
    let arr = pre_arr;
    ...
    if (post_arr_min.length > 1)
        console.log("Effect has more then 1 post script", post_arr, "for ratio", root);
...
function level_to_ratio(fulltext: string): { values: number | number[]; apply: '%' | undefined; units: string; } {
    ...
    const arr = leveling.split(/[/−]/).map((x: string) => {
        ...
        if (Number.isFinite(n)) {
            units = units + split[1] || '';
            console.log(`[INFO] Stat leveling split '${leveling}' needed to split value: '${x}' into: [${split}] n=${n} units="${units}"`);

            return n;
        };
        console.log(`[WARN] Stat leveling '${leveling}' has failed (${fail_count})`);

...
const stylesTemplate = (perksLookup: { [key: string]: string }, Runes: object) => `<template>
...
${Object.values(perksLookup).map(x => `import ${x} from './perks/${x}.vue'`).join('\n')}
const perkComponents = ${JSON.stringify(Runes).replace(/\"/g, '')};
</script>`

const stylesTemplate2 = (a: string[], b: string[], c: string[], d: string[]) => `<template>
    <PerkStyle>
        <template #keystone>
${a.map(x => `            <${x}></${x}>`).join('\n')}
        </template>
...
<script setup lang="ts">
import PerkStyle from './PerkStyle.vue';
${[a, b, c, d].flat().map(x => `import ${x} from './perks/${x}.vue'`).join('\n')}
</script>`
...
async function readPerks() {
    ...
    for (const styles of Object.values(perks.styles)) {
        ...
        // }
        for (const rune of [styles.slots.KeyStone, styles.slots.Mixed1, styles.slots.Mixed2, styles.slots.Mixed3].flat()) {
